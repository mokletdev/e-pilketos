...
-- CreateTable
CREATE TABLE `Topik_vote` (
    `id` VARCHAR(191) NOT NULL,
    `situasi` LONGTEXT NOT NULL,
    `pertanyaan` VARCHAR(191) NOT NULL,
    `jawaban` VARCHAR(191) NOT NULL,
    `vote_session_candidate_id` VARCHAR(191) NOT NULL,
...
CREATE TABLE `Tanggapan` (
    ...
    `topik_vote_id` VARCHAR(191) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Topik_vote` ADD CONSTRAINT `Topik_vote_vote_session_candidate_id_fkey` FOREIGN KEY (`vote_session_candidate_id`) REFERENCES `Vote_session_candidate`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Tanggapan` ADD CONSTRAINT `Tanggapan_topik_vote_id_fkey` FOREIGN KEY (`topik_vote_id`) REFERENCES `Topik_vote`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
