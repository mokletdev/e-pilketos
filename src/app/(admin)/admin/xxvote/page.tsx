...
import {
  getVoteSessionsForTransfer,
  transferVote,
  type TransferVoteData,
  type VoteSessionWithCounts,
} from "@/utils/forbidden-vote";

export default function TransferVotePage() {
  const [voteSessions, setVoteSessions] = useState<VoteSessionWithCounts[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchSessions() {
      try {
        const res = await getVoteSessionsForTransfer();
        if (res) {
          setVoteSessions(res);
        } else {
          setMessage({ type: "error", text: res || "Gagal ambil data" });
        }
      } catch (e) {
        setMessage({ type: "error", text: "Error mengambil data" });
      }
    }
  ...
  const selectedSessionData = voteSessions?.find(
  ...
  const handleTransferVote = async () => {
    ...
    if (selectedFromCandidate === selectedToCandidate) {
      setMessage({
        type: "error",
        text: "Kandidat pengirim dan penerima tidak boleh sama",
      });
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: "Terjadi kesalahan saat transfer vote",
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Kelola dan transfer vote antar kandidat dalam sesi pemungutan suara
            aktif
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          {voteSessions?.map((session) => {
            ...
            const totalVotes = session.vote_session_candidate.reduce(
              (sum, vsc) => sum + vsc.currentVotes,
              0,
            );
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    ...
                    <div className="flex justify-between text-sm text-gray-600">
                      ...
                      <span>
                        Kandidat: {session.vote_session_candidate.length}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center space-x-4">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">
                      {fromCandidateData?.currentVotes} →{" "}
                      {fromCandidateData?.currentVotes! -
                  ...
                  <div className="text-center">
                    <p className="font-medium text-gray-900">
                      {
                        selectedSessionData?.vote_session_candidate.find(
                          (vsc) => vsc.candidate_id === selectedToCandidate,
                        )?.candidate.name
                      }
                    </p>
                    <p className="text-sm text-gray-600">
